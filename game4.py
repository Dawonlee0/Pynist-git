...
def stage4():
    ...
    def move_bullets():
        ...
        for bullet in bullets:
            ...
            # 사용자와 총알 충돌 확인
            if bullet_coords[2] >= canvas.coords(user)[0] and bullet_coords[0] <= canvas.coords(user)[2] and \
                    bullet_coords[3] >= canvas.coords(user)[1] and bullet_coords[1] <= canvas.coords(user)[3]:
            ...
            global position
            if position == 1 or position == 5 or position == 7:
                # 총알이 화면 밖으로 벗어나면 삭제
                ...
                # 총알이 화면 밖으로 벗어나면 삭제
                if bullet_coords[2] >= 400:
                    bullets_to_remove.append(bullet)  # 범위를 벗어난 총알 추가

            elif position == 3 or position == 6 or position == 8:
                # 총알이 화면 밖으로 벗어나면 삭제
    ...
    def shoot_bullet():
        ...
        if position == 1:
            x = random.randint(0, 360)  # x 좌표 랜덤 생성
            y = 0
            bullet = canvas.create_rectangle(x, y, x + 40, y - 40, fill="red")  # 총알 생성
            bullets[bullet] = (0, speed)  # 총알의 이동 속도 설정

        elif position == 2:
            x = 0
            y = random.randint(0, 360) # y 좌표 랜덤 생성
            bullet = canvas.create_rectangle(x, y, x - 40, y + 40, fill="red")  # 총알 생성
            bullets[bullet] = (speed, 0)  # 총알의 이동 속도 설정
            ...
            x = 0
            y = random.randint(0, 160)  # y 좌표 랜덤 생성
            bullet = canvas.create_rectangle(x, y, x - 40, y + 40, fill="red")  # 총알 생성
            bullets[bullet] = (speed, speed)  # 총알의 이동 속도 설정

        elif position == 6:
            x = 0
            y = random.randint(200, 360)  # y 좌표 랜덤 생성
            bullet = canvas.create_rectangle(x, y, x - 40, y + 40, fill="red")  # 총알 생성
            bullets[bullet] = (speed, -speed)  # 총알의 이동 속도 설정
    ...
    def shoot_bullets(count):
        ...
        shoot_bullet()  # 총알 발사
        root.after(700, lambda: shoot_bullets(count + 1))  # 2초 간격으로 총알 발사
